...
//  BlockchainSdkTests
//
//  Created by Andrew Son on 02/03/21.
...
class BitcoinTests: XCTestCase {
    ...
    func testBtcTxBuilder() {
        ...
        let compressedPubkey = Secp256k1Utils.convertKeyToCompressed(pubkey)
        XCTAssertNotNil(compressedPubkey)
        
        let signature = Data(hex: "")
        ...
        let seg: SegWitAddress = addressService.bech32.makeAddress(from: pubkey)
        ...
        let buildToSignResult = txBuilder.buildForSign(transaction: transaction)
        let signedTx = txBuilder.buildForSend(transaction: transaction, signature: signature, hashesCount: buildToSignResult?.count ?? 0)
        
        XCTAssertEqual(buildToSignResult?.filter { $0 == expectedHashToSign1 || $0 == expectedHashToSign2 }.count, 2)
        XCTAssertEqual(signedTx, expectedSignedTransaction)
    }
    
    private func prepareTwoUnspentOutputs(for addresses: [BitcoinCore.Address]) -> [BtcTx] {
        ...
        let utxo1 = BtcTx(tx_hash: "8b907ad6ee8c6b1d25375ce9696089fec400851ca46260927d04892ec88807ef",
                          tx_output_n: 0,
                          value: 39920000,
                          script: utxo1Script!.hex)
        let utxo2 = BtcTx(tx_hash: "cffea3f46c73d61c6ed1296494b3c85e9f498629a32d67367a0d9e1bafdd05df",
                          tx_output_n: 1,
                          value: 12210000,
                          script: utxo2Script!.hex)
        return [utxo2, utxo1]
    }
}
